...
export default class Noise{
    ...
    perlin1D(x) {
        ...
        return this.lerp(u, this.grad1D( a, x     ),
            this.grad1D( b, x - 1 ));
    }
    ...
    perlin2D(x, y) {
        ...
        let aa = this.p[this.p[ X     ]+ Y        ],
            ab = this.p[this.p[ X     ]+ Y + 1    ],
            ba = this.p[this.p[ X + 1 ]+ Y        ],
            bb = this.p[this.p[ X + 1 ]+ Y + 1    ];

        // add blended results from 4 corners of square
        return this.lerp ( v, this.lerp( u, this.grad2D( aa, x    , y     ),
                this.grad2D( ba, x - 1, y     )),
            this.lerp( u, this.grad2D( ab, x    , y - 1 ),
                             this.grad2D( bb, x - 1, y - 1 )));
    ...
    perlin3D(x, y, z) {
        ...
        let aaa = this.p[this.p[this.p[ X		 ]+ Y		  ]+ Z		 ],
            aab = this.p[this.p[this.p[ X		 ]+ Y		  ]+ Z + 1 ],
            aba = this.p[this.p[this.p[ X		 ]+ Y + 1 ]+ Z		 ],
            abb = this.p[this.p[this.p[ X		 ]+ Y + 1	]+ Z + 1 ],
            baa = this.p[this.p[this.p[ X + 1 ]+ Y		  ]+ Z		 ],
            bab = this.p[this.p[this.p[ X + 1 ]+ Y		  ]+ Z + 1 ],
            bba = this.p[this.p[this.p[ X + 1 ]+ Y + 1	]+ Z	   ],
            bbb = this.p[this.p[this.p[ X + 1 ]+ Y + 1	]+ Z + 1 ];

        // add blended results from 8 corners of cube
        return this.lerp ( w, this.lerp ( v, this.lerp( u,  this.grad3D( aaa, x    , y    , z     ),
                    this.grad3D( baa, x - 1, y    , z     )),
                this.lerp( u,  this.grad3D( aba, x    , y - 1, z     ),
                                       this.grad3D( bba, x - 1, y - 1, z     ))),
            this.lerp ( v, this.lerp( u,  this.grad3D( aab, x    , y    , z - 1 ),
                                 this.grad3D( bab, x - 1, y    , z - 1 )),
                this.lerp( u,  this.grad3D( abb, x    , y - 1, z - 1 ),
                                       this.grad3D( bbb, x - 1, y - 1, z - 1 ))));
    };
